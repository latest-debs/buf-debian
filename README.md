![buf for Debian](.github/readme-header.png)

# buf for Debian

[![Release](https://img.shields.io/github/v/release/latest-debs/buf-debian)](https://github.com/latest-debs/buf-debian/releases)
[![Build](https://github.com/latest-debs/buf-debian/actions/workflows/release.yml/badge.svg)](../../actions)
...
sudo apt update
sudo apt install buf
```

Or download a `.deb` from the [Releases](https://github.com/latest-debs/buf-debian/releases) page:

```sh
sudo apt install ./buf_*.deb
```

## Verify

```sh
apt-cache policy buf
buf --version
```
...
- amd64, arm64, armhf, i386, armel, loong64, ppc64el, riscv64, s390x —
  whichever architectures bufbuild/buf actually publishes a Linux
  binary for

## Building

Run the [Build buf for Debian](../../actions) workflow on GitHub with the
...
For issues with buf itself, see
